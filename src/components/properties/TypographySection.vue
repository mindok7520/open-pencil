    ...
    <div data-test-id="typography-section" class="border-b border-border px-3 py-2">
      ...
      <div class="mb-1.5 flex items-center gap-1.5">
        <FontPicker
          ...
          :model-value="ctx.node.fontFamily"
          @select="ctx.setFamily"
        />
        <Tip
          v-if="ctx.hasMissingFonts"
          :label="
            'Missing font' +
      ...
      <div class="mb-1.5 flex gap-1.5">
        <AppSelect
          :model-value="ctx.node.fontWeight"
          :options="ctx.weights"
          @update:model-value="ctx.setWeight(+$event)"
        />
        <ScrubInput
          class="flex-1"
          :model-value="ctx.node.fontSize"
          :min="1"
          :max="1000"
      ...
      <div class="mb-1.5 flex gap-1.5">
        <ScrubInput
          class="flex-1"
          :model-value="ctx.node.lineHeight ?? Math.round((ctx.node.fontSize || 14) * 1.2)"
          :min="0"
        ...
        <ScrubInput
          class="flex-1"
          suffix="%"
          :model-value="ctx.node.letterSpacing"
      ...
      <div class="flex items-center gap-3">
        <ToggleGroupRoot
          type="single"
          class="flex gap-0.5"
          :model-value="ctx.node.textAlignHorizontal"
          @update:model-value="
            (val: AcceptableValue) => {
        ...
        <ToggleGroupRoot
          type="multiple"
          class="flex gap-0.5"
          :model-value="ctx.activeFormatting"
          @update:model-value="
            (val: AcceptableValue | AcceptableValue[]) => {
